...
3. **The report's stakeholder-reflect file** at `~/haku-work-reflections/managing-down/<slug>.md`. Read the full file. Pay attention to entries on accomplishments, struggles, scope, and feedback received.
...
- **`user-profile`** — if `~/haku-work-reflections/profile.md` exists, read it before drafting. It carries the user's level, which calibrates *what bar to argue against* (a Director writing a Staff promo packet has a different framing than a manager writing a Senior packet). The user shouldn't have to re-explain.
