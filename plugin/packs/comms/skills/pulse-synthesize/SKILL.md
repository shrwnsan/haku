...
1. Read `~/haku-work-reflections/strategy/<area-slug>.md` if present — it's the interpretive frame.
2. Read all pulse files in `~/haku-work-reflections/pulses/<area-slug>/` falling within the time window. Sort chronologically.
3. Optional: read `~/haku-work-reflections/profile.md` for the user's strategic focus context.
