...
Guide a single reflection session about a registered stakeholder. The output is a dated entry appended to `~/haku-work-reflections/<category>/<slug>.md` for one or more questions from the question library.
...
1. **Which stakeholder.** Either named explicitly ("John") or implied ("my manager"). Resolve via `~/haku-work-reflections/stakeholders.json`.
