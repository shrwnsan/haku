---
title: "LEAN-PLAN — haku architecture (the bettersense v2 redesign)"
---
...
- **D7 — ship-review review outcomes (feat/gates-hooks, round 1).** Three rulings from adversarial review: (1) the CI judge-gate flag `HAS_JUDGE` is declared at workflow level, because job-level `env` is not in scope for that job's own `if:` — the job-level form silently never runs, the worst failure mode for a governance gate. D7 correction (feat/portable-emitter): workflow-level `env` is necessary but not sufficient — a job-level `if` cannot reference the `env` context at all; GitHub rejects the entire workflow at parse time (`Unrecognized named-value: 'env'`), which had silently zeroed every CI run since the gates-hooks merge (runs created, conclusion failure, zero jobs). The gate therefore sits on each `routing-live` step, where step-level `if` does see `env` — the governance lesson generalizes: gate placement must be validated against the platform's actual parse semantics, not the documented context table alone (the docs table is where the first ruling came from, and it was right about availability and still missed the job-`if` restriction). (2) This repo's decision log is `docs/LEAN-PLAN.md` §8 and stays a single home: P3 accepts repeatable `--decision-doc <path>` for file-based logs, and CI passes `--decision-doc docs/LEAN-PLAN.md` — no `decisions/` directory fork; `FLAG:`/`SHADOW:` tokens are declared by a §8 D-entry. (3) The local PreToolUse hook fails open on infrastructure errors (spawn failure → note + proceed); CI's fail-closed predicates are the enforced gate. Accepted debt: P1's `\bD[0-9]+\b` match is loose (any D-number passes); verifying the cited entry exists in the log is the follow-up once P3's declaration home is exercised. Enforcement is PR-shaped — the ship-review job runs on `pull_request` only, so merges to `main` go through PRs.
- **D8 — haku rename ships as rename-with-migration; routing boundaries re-cut (feat/body-diet).** (1) User-facing homes rename with a migration path, not a cliff: `~/bettersense-work-reflections/` → `~/haku-work-reflections/` (`$HAKU_WORK_REFLECTIONS_HOME`), `$BETTERSENSE_TEAM_HOME`/`~/bettersense-team/` → `$HAKU_TEAM_HOME`/`~/haku-team/`. `doctor` check 6 detects a legacy root and offers the `mv` (move, never copy-duplicate; ask rather than merge); `user-profile` offers the same before creating a fresh root. Install/uninstall symlink matching switches from `*bettersense*` to `*haku*` targets — required for the scripts to function at all. (2) Reducer-vs-discovery (the research-pack known miss): `the-reducer` owns ONLY the whether-to-use-AI call and hands off once the whether is settled; `the-discovery-facilitator` owns problem scoping even when AI is already assumed. The `discovery` case now carries a `source` citing this entry. (3) All frontmatter descriptions rewritten ≤60 words in haku voice — this is eval-surface change, so the routing baseline regenerates via a decision-citing commit per D4/D5 at branch close. D8 execution note (feat/body-diet close-out): all pack bodies dieted to ≤800 words and descriptions to ≤60 words, verified across the full branch diff; CLAUDE.md data-root paths updated to haku with the legacy-migration note; routing dry-run, all eight pack dry-runs, and lint-gates green at this commit. D8 baseline regen @18343d7: N=3 on glm-5.3-flash (claude@2.1.128) ran 11/11, 10/11, 11/11 — `capture` flipped once and is recorded flaky in baseline.json, within the D4 single-flip tolerance; watch it at the next regen before touching the case. D8 review round 1 (Gilfoyle): the initial mechanical budget check covered only the uncommitted set and missed three stage-1 files (the-discovery-facilitator 983w, doctor 846w, user-profile 838w) — all dieted ≤800 and the check re-run across the full branch diff; uninstall scripts (sh + ps1) now detect pre-rename `*bettersense*` symlinks and offer removal, so legacy installs aren't stranded; MANIFEST.md data-root section updated to haku with the migration note.
- **D9 — portable emit: Agent Plugins spec v1.0.0, skills-only, tag-pinned version (feat/portable-emitter).** The cross-client portable format (agent-plugins.org) defines exactly two component types — skills and MCP servers — and holds agents/hooks/commands outside v1 as too client-specific. `scripts/emit-portable.mjs` therefore emits skills only: core 8 by default, `--pack <name>` (repeatable) flattens a pack's skills alongside. Each skill directory is copied verbatim after Agent Skills conformance checks (frontmatter `name` === directory name, charset, description ≤ 1024 chars and single-line); a name collision fails the emit. Accepted degradation, stated in the emitted README: gate agents + `gates/` are absent (the decide → spec → evidence loop degrades to un-gated guidance), ship-review P1–P4 stay repo/CI-only (a pull-based MCP review tool wrapping P1–P4 is the planned follow-up), monitors and pack agents drop with the same rationale. The emitted `plugin.json` takes identity fields from the marketplace entry and `version` from the latest git tag — tags stay the single version source, and the stale `0.4.0` strings in both repo manifests were synced to the tag in this branch. `dist/agent-plugins/` (the core emit) is committed, and the CI `portable-drift` job re-emits with `--check`, failing on any difference — a hand-edited dist or a skill change without re-emit breaks the build. Spec 1.1.0 exists as a working draft (version-string deltas only at time of writing); revisit on publication.

- **D10 — routing routers are pluggable; the System One model ships as an eval provider, not the judge (feat/jev-router).** The runner gains `--router jev` alongside the default claude CLI judge: the same pick-from-N judgment is posed to TypeSafe's Jev as a choice question whose `criteria` keys are the candidate names and whose values are the candidate descriptions themselves, with `none` an explicit option instead of a prompt convention. Rationale: routing is state-bound classification — the answer lives in the provided descriptions — which is exactly where a decision model is supposed to hold up, and `baseline.json` (D4/D6) already pins the claude column for a like-for-like comparison. Jev stays eval-only: it does not become the CI judge or a production router here — the judge's single source of truth remains `baseline.json` per D6 — until a decision-citing regen shows it beating the pinned judge. The provider pins the exact served version (`jev-1.13.0`), not the floating `jev-latest` alias, so an N-run aggregate stays reproducible (pinning discipline per D4/D6). Fresh N=3 on core at this commit: 10/11, 10/11, 10/11 vs the pinned 11/11, 10/11, 11/11; the miss is `capture` firing its must-not-fire neighbour `decision-log`, deterministic across runs (the same case already recorded flaky for the LLM judge). Attribution caveat, stated deliberately: the Jev run changes both the model and the elicitation format (structured choice over descriptions vs prose catalog + JSON-last-line), so the deterministic miss cannot be cleanly attributed to description quality without a format-controlled ablation — the honest read is that Jev surfaces a latent `capture`/`decision-log` boundary weakness that the LLM judge resolves stochastically. Pack-scoped single runs (informational, no pinned baseline): 108/116 summed across the eight pack runs, where each run re-poses the 11 core cases plus that pack's own (28 distinct pack-only cases across the eight). Auth is environmental — the OneCLI gateway injects at the proxy boundary, or `TYPESAFE_API_KEY` is read from the environment where no gateway exists; no credential material in-repo, and CI does not run the Jev path (no secret is declared for it), so the `routing-live` job and its `HAS_JUDGE` gate (D7) are unchanged. Gilfoyle round 1 (feat/jev-router): fixed version pin, 429/529/5xx retry with backoff, none-collision + 255-cap validation hoisted to dataset validation, `--router=` equals-form parsing, `--model`-ignored warning under Jev; remaining accepted debt — `make-baseline.mjs` cannot aggregate Jev runs (hardcodes the claude invocation), deferred until a Jev baseline regen is actually decided.

- **D11 — docs served via GitHub Pages from `main` `/docs`, built-in Jekyll (2026-09-21).** The docs corpus stays plain markdown in-repo (D3), but retiring the Starlight site (the PR #3 fold) left no browsable URL — the corpus was readable only inside github.com file views. Pages-from-main restores one at zero CI: `docs/_config.yml` pins the `jekyll-theme-primer` theme with a defaults entry supplying the layout, every docs markdown file gains a minimal front matter block (title only — Jekyll processes only front-matter'd files, so without this the corpus would be static-copied as raw text and the site root would 404), and `docs/index.md` becomes the site root while `docs/README.md` remains the github.com file-view map — two entry points for two audiences, drift accepted and cheap to reconcile. First-stab scope by design: no standalone domain per the standing web-presence call (folio OSS section first; the sherwintechico.com subdomain stays parked as "perhaps"), reversible in one toggle, and no routing surface touched (P2 n/a; the front-matter pass is content-only).
