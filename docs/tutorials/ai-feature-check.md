...
**Time:** ~10 minutes
**Skills used:** `the-reducer` (core gate agent) → `the-scientist` (evals pack) → `the-eval-designer` (evals pack)
...
**Prerequisites:** haku installed — see [the install guide](../install.md). `the-reducer` is always installed; steps 2 and 3 need the evals pack (`scripts/install.sh --pack evals`).
...
```
/haku:start
I want to build an AI feature that auto-drafts support ticket replies. The VP
of CS thinks this will fix our 4-hour response time. Should we build it?
```
...
You haven't built anything yet. You've avoided six weeks of engineering on the wrong problem — and the intent is now logged, ready for the spec gate if you proceed.

---

## Step 2: Validate the smaller scope with the-scientist

The reducer's verdict: if you do pursue the draft assistant for the remaining 40% of delay, do a feasibility pass first. With the evals pack installed, you run:

```
/haku:the-scientist
Assuming we fix routing separately, I still want to explore the LLM draft
assistant. Help me figure out if it's feasible before committing engineering time.
```
...
```
/haku:the-eval-designer
I want to build a minimal eval system for the support draft assistant. I need
to be able to catch regressions when I update the prompt or the model.
```
...
- [Packs](../packs.md) — what the evals pack installs and when to add it
- [Core loop and gates](../gates.md) — how the decide → spec → evidence gates enforce this discipline on real work
