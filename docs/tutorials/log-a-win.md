...
**Time:** ~8 minutes
**Skills used:** `/haku:wins-log` (core) → `wins-curate` (people pack)
...
**Prerequisites:** haku installed — see [the install guide](../install.md). This tutorial's curation step needs the people pack (`scripts/install.sh --pack people`); logging works in a default install.
...
```
/haku:wins-log
I caught a dependency issue between the platform team and the data team three
weeks before the Q2 launch, got both teams aligned on a workaround, and the
launch went out on time.
```
...
The skill writes the entry to `~/haku-work-reflections/wins.md`:
...
```
/haku:wins-log
I mentored a new junior PM through her first exec briefing. She ran it solo
and the VP called it one of the clearest briefings he'd seen from someone at her level.
```
...
Six weeks later, preparing for an interview (people pack installed):
...
> **Tip:** The reason most wins logs stay empty: the wins feel small in the moment, or you forget. The `wins-due` skill (people pack), run on a Friday schedule — or as part of `/haku:weekly` — asks one category-probing question per week: *"any mentorship this week?"*, *"any judgment calls?"* — that surfaces entries you'd otherwise skip.

---

## Where to go next

- [The Friday leadership review](friday-review.md) — the weekly routine that keeps the wins log current
- [Build a promotion case](promo-case.md) — taking the log through a full promo packet
