...
**Time:** ~10 minutes
**Skills used:** `stakeholder-register` → `stakeholder-reflect` → `stakeholder-synthesize` (all stakeholder pack)
**Who this is for:** Anyone new to haku who works with people they need to manage up, across, or down.
...
**Prerequisites:** haku installed — see [the install guide](../install.md) — plus the stakeholder pack (`scripts/install.sh --pack stakeholder`). If you haven't set up your profile yet, run `/haku:user-profile` first — it sets up your private data directory.
...
> *"I'll create `~/haku-work-reflections/managing-across/priya-mehta.md` with this context. Does that look right?"*

You confirm. File is created. One-time setup: done.

> **Note:** Stakeholder files live in your reflections root (`~/haku-work-reflections/`, `$HAKU_WORK_REFLECTIONS_HOME` to override) alongside the rest of your private memory — with `stakeholders.json` as the register, `managing-up|across|down/<slug>.md` per person, and `archive/<category>/<slug>.md` when a relationship ends. They never touch the shared team workspace.
...
- [Log a win and turn it into a story](log-a-win.md) — the wins system, from capture to promo packet
- [The Friday leadership review](friday-review.md) — a weekly routine that uses `stakeholder-due` to keep reflections current
- [Packs](../packs.md) — the full stakeholder pack inventory
