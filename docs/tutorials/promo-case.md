...
**Time:** ~12 minutes
**Skills used:** `report-career-architect` → `wins-curate` → `report-promo-case` (all people pack)
...
**Prerequisites:** haku installed — see [the install guide](../install.md) — plus the people pack (`scripts/install.sh --pack people`). The tutorial works without a wins log for the report, but if you've been logging their wins with `wins-log`, the output will be substantially richer.
...
```
/haku:wins-curate
Help me build a promo evidence map for Kofi. I want to know what I have and
what I'm missing.
```
...
- [Navigate an underperformance situation](underperformance.md) — the other end of the performance management spectrum
- [Packs](../packs.md) — the full people pack inventory, including `the-career-coach`
