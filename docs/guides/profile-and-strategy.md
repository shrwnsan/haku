...
Two files dramatically improve output quality across the whole plugin. Both are optional—skills work without them—but the difference is noticeable from the first invocation.

## `profile.md` — who you are

Created by `user-profile` (core). Lives at `~/haku-work-reflections/profile.md`.
...
**Skills that read it automatically:** the core `the-spec-writer` gate agent, plus installed pack skills—the translator and the explainer (comms), report-promo-case, report-career-architect, coaching-mode, feedback-frameworks (people), metrics-design (leadership).

What changes: the spec writer knows your stack defaults, the translator knows your audience register, coaching-mode knows your default directness preference.

### Maintenance

- Create once with `/haku:user-profile`—5-10 minute interview
...
Created by `strategy-doc` (core). Lives at `~/haku-work-reflections/strategy/<area-slug>.md`.
...
**Skills that read it automatically:** the core `the-spec-writer`, plus installed pack skills—prioritization-frameworks and metrics-design (leadership), product-pulse and pulse-synthesize (comms).
...
- The strategy stops matching what you're actually working on (a signal from `patterns-watch`, leadership pack)
