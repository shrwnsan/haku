...
> **Pack note:** the four skills used here—`stakeholder-register`, `stakeholder-reflect`, `stakeholder-due`, `stakeholder-synthesize`—ship in the **stakeholder pack** (`scripts/install.sh --pack stakeholder`). See [Packs](../packs.md).
...
Confirms before writing: *"Here's the file I'll create at `~/haku-work-reflections/managing-up/trae-north.md`. Anything to change?"*
...
- **It composes.** After reading the synthesis, you can run—with the relevant packs installed:
  - `/haku:one-on-one-prep` (people pack)—for the actual skip-level agenda
  - `/haku:feedback-frameworks` (people pack)—if feedback emerges from the synthesis
  - `/haku:coaching-mode` (people pack)—if a development conversation is what the synthesis points to
